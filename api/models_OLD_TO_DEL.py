...
#     SessionSettings
# )
# from shop.utils.pricing import round_to_tick, decimals_from_tick, make_idempotency_hash
...
#         """
#         Try to return an integer epoch ms. Accepts int, str(int),
#         and leaves non-numeric values as-is.
#         """
#         if value is None:
#             return value
...
#     We try common field names and parse.
#     """
#     if not tp_obj:
#         return {}
#     candidates = ("plan", "json", "plan_json", "planJson",
#                   "data", "content", "body", "value", "parameters", "config")
#     raw = None
...
#     # optional per-session override JSON (if present in your model)
#     session_norm = {}
...
#     # from Study → TradingPlan
#     study_norm = {}
#     study = getattr(session, "study", None)
#     if study:
#         tp_obj = None
#         if hasattr(study, "tradingPlan") and getattr(study, "tradingPlan"):
#             tp_obj = getattr(study, "tradingPlan")
#         elif hasattr(study, "trading_plan") and getattr(study, "trading_plan"):
#             tp_obj = getattr(study, "trading_plan")
...
#         return 3
#     import decimal
#     d = decimal.Decimal(str(tick_size)).normalize()
#     return max(0, -d.as_tuple().exponent)
# def _q_price(p, tick_size):
#     if not tick_size:
#         return float(p)
#     from shop.utils.pricing import quantize_price  # your helper
#     return float(quantize_price(p, tick_size))
# # endpoints examples
...
#             #),
#             # List indicators available to this session (via its study)
#             re_path(
...
#             # List indicator values for this session’s candles (optionally filter)
#             re_path(
...
#                 % (self._meta.resource_name, trailing_slash()),
#                 self.wrap_view('generate_potential_orders_by_timestamp'),
...
#         from shop.session_models import TradingSession
...
#         and legacy GET /api/v1/sessions/<pk>/indicatorsValues/
#         Mirrors StudyResource.get_study_indicators_values style & keys.
#         """
#         self.method_check(request, allowed=['get'])
#         self.is_authenticated(request)
#         self.throttle_check(request)

#         from shop.session_models import TradingSession, SessionStockDataIndicatorValue
...
#         out = []
#         for iv in qs:
#             # value: try parse JSON and round numeric "value" to 3 decimals
#             val_str = iv.value
#             try:
...
#             except Exception:
#                 # if plain numeric string, we can still round a bit for cleanliness
#                 try:
#                     num = float(val_str)
...
#             out.append({
#                 # Keep keys identical to Study version:
#                 'id': iv.id,
...
#     # ---------- Potential Orders: Step 1 (Generate) ----------
...
#     def _resolve_trading_plan_arrays(self, session, body):
#         """
#         Priority:
#         1) Body overrides: lp_offsets, stop_losses, take_profits
#         2) First StudyTradingPlan for session.study → tradingPlan.tradingPlanParams
#         expects: {"LPoffset":[...], "stopLoss":[...], "takeProfit":[...]}
#         """
#         lp = body.get('lp_offsets')
#         sl = body.get('stop_losses')
#         tp = body.get('take_profits')
#         plan_version = body.get('plan_version', 'latest')

#         if lp is None or sl is None or tp is None:
#             plan_link = StudyTradingPlan.objects.filter(study=session.study).select_related('tradingPlan').first()
#             if not plan_link or not plan_link.tradingPlan or not plan_link.tradingPlan.tradingPlanParams:
#                 raise ImmediateHttpResponse(HttpResponseBadRequest(
#                     "Trading plan arrays missing: provide lp_offsets/stop_losses/take_profits or configure StudyTradingPlan"
#                 ))
#             params = plan_link.tradingPlan.tradingPlanParams
#             lp = lp if lp is not None else params.get('LPoffset', [])
#             sl = sl if sl is not None else params.get('stopLoss', [])
#             tp = tp if tp is not None else params.get('takeProfit', [])

#         if not (lp and sl and tp):
#             raise ImmediateHttpResponse(HttpResponseBadRequest("Empty plan arrays (lp_offsets/stop_losses/take_profits)"))

#         # Normalize to Decimal (tp kept as int if you prefer)
#         LPO = [Decimal(str(x)) for x in lp]
#         SL  = [Decimal(str(x)) for x in sl]
#         # tp is TP:SL multiple — typically integer, keep as Decimal for math then cast when saving to model
#         TP  = [Decimal(str(x)) for x in tp]

#         return LPO, SL, TP, plan_version

#     def _resolve_market_meta(self, session, body):
#         """
#         Resolve tick_size/lot_step for quantization & response formatting.
#         If tick_size is missing, we skip quantization and default display decimals to 6.
#         """
...
#         """
#         Read indicator value for a specific candle via SessionStockDataIndicatorValue.
#         The value field is JSON like {"value": number}.
#         """
#         si = (StudyIndicator.objects
#             .filter(study=session.study, mask=mask)
#             .order_by('id')
#             .first())
#         if not si:
#             raise ImmediateHttpResponse(HttpResponseBadRequest(f"StudyIndicator with mask '{mask}' not found"))

#         iv = (SessionStockDataIndicatorValue.objects
#             .filter(sessionStockDataItem=candle, studyIndicator=si)
#             .first())
#         if not iv:
#             raise ImmediateHttpResponse(HttpResponseBadRequest(f"Indicator '{mask}' value not found for this candle"))
...
#             v = None
#         if v is None:
#             raise ImmediateHttpResponse(HttpResponseBadRequest(f"Indicator '{mask}' value is empty for this candle"))
...
#         """
#         Core logic: build BUY/SELL × LPO × SLatr × TP combinations,
#         freeze prices, create/reuse SessionPotentialOrder rows, return JSON.
#         """
#         LPO, SL, TP, plan_version = self._resolve_trading_plan_arrays(session, body)
...
#         # sATR value (from indicators table) unless provided explicitly
#         satr = body.get('satr')
...
#                 for lpo in LPO:
#                     d_entry = (lpo * satr)
#                     entry = close - d_entry if direction == 'BUY' else close + d_entry

#                     for slatr in SL:
#                         d_sl = (slatr * satr)
#                         stop = entry - d_sl if direction == 'BUY' else entry + d_sl

#                         for tpmult in TP:
#                             d_tp = (tpmult * d_sl)
#                             take = entry + d_tp if direction == 'BUY' else entry - d_tp

#                             # Quantize only if we have a tick_size
#                             if quantize and tick_size:
#                                 entry_q = round_to_tick(entry, tick_size)
#                                 stop_q  = round_to_tick(stop,  tick_size)
#                                 take_q  = round_to_tick(take,  tick_size)
#                             else:
#                                 entry_q, stop_q, take_q = entry, stop, take

#                             idem = make_idempotency_hash(
#                                 session.id, candle.id, direction, lpo, slatr, tpmult, plan_version
#                             )
...
#                                         direction=direction,
#                                         # stored as parameters & frozen prices
#                                         lpOffset=lpo,
#                                         slATR=slatr,
#                                         tp=int(tpmult),  # tp as integer multiple
#                                         limitPrice=entry_q,
#                                         stopPrice=stop_q,
#                                         takeProfitPrice=take_q,
#                                         # optional fields if you maintain them
#                                         prediction=None,
#                                         decision='NONE',
#                                     )
#                                 )
...
#         from shop.session_models import TradingSession, SessionStockData, SessionPotentialOrder
...
#         # resolve plan (Study -> TradingPlan)
#         try:
#             plan_used, plan_source, _ = _resolve_trading_plan_for_session(session, body)
#         except ValueError as e:
#             return self.create_response(request, {"error": str(e)}, http.HttpBadRequest)

#         dry_run = bool(body.get("dry_run", False))
#         quantize = bool(body.get("quantize", False))
#         tick_size = body.get("tick_size")
#         lot_step = body.get("lot_step", 1)

#         # fallback to broker defaults if not provided
#         if tick_size is None and getattr(session, "broker_settings", None):
#             tick_size = session.broker_settings.get("tick_size")
#         if lot_step is None and getattr(session, "broker_settings", None):
#             lot_step = session.broker_settings.get("lot_step", 1)

#         # candle & inputs
#         candle = get_object_or_404(SessionStockData, trading_session=session, timestamp=ts)
#         close = float(candle.close)
#         satr = _get_satr_for_candle(session, candle, default_mask="sATR")
#         price_decimals = _price_decimals_from_tick(tick_size)

#         results = []
#         # build combinations
#         for lp in plan_used["lp_offsets"]:
#             for sl in plan_used["stop_losses"]:
#                 for tp in plan_used["take_profits"]:
#                     # BUY
#                     buy_limit = close - lp * satr
#                     buy_stop  = buy_limit - sl * satr
#                     buy_tp    = buy_limit + (sl * satr) * tp

#                     # SELL
#                     sell_limit = close + lp * satr
#                     sell_stop  = sell_limit + sl * satr
#                     sell_tp    = sell_limit - (sl * satr) * tp

#                     combos = [
#                         ("BUY",  buy_limit,  buy_stop,  buy_tp),
#                         ("SELL", sell_limit, sell_stop, sell_tp),
#                     ]
#                     for side, lim, stp, tpp in combos:
#                         if quantize:
#                             lim = _q_price(lim, tick_size)
#                             stp = _q_price(stp, tick_size)
#                             tpp = _q_price(tpp, tick_size)
...
#                             "plan_used": plan_used,
#                         }
#                         # idempotency hash (same input -> same hash)
#                         hsrc = f"{session.id}|{candle.id}|{side}|{lp}|{sl}|{tp}|{tick_size}|{lot_step}|{quantize}|{close}|{satr}"
#                         meta["idempotency_hash"] = hashlib.sha256(hsrc.encode("utf-8")).hexdigest()
...
#                                 "slATR": sl,
#                                 "tp": tp,
#                                 "limitPrice": round(lim, price_decimals),
#                                 "stopPrice": round(stp, price_decimals),
#                                 "takeProfitPrice": round(tpp, price_decimals),
#                                 "status": "DRY_RUN",
#                                 "format": {"price_decimals": price_decimals},
#                                 "meta": meta,
#                             })
#                         else:
#                             # persist a real SessionPotentialOrder
...
#     def generate_potential_orders_by_timestamp(self, request, **kwargs):
#         self.method_check(request, allowed=['post'])
#         self.is_authenticated(request)
#         self.throttle_check(request)
...
#         if not ts:
#             raise ImmediateHttpResponse(HttpResponseBadRequest("timestamp query param required"))
